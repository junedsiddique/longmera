...
mix_means <- function(object, marginals, tx.contrast0=NULL, tx.contrast1=NULL,
  ...
  if(sum(!is.null(tx.contrast1), !is.null(tx.contrast0))==2){
    tx.result[1,] <- calculate_z_statistics(tx.output$c1$mean.prob,
                                               tx.output$c1$se)

    tx.result[2,] <- calculate_z_statistics(tx.output$c0$mean.prob,
                                               tx.output$c0$se)

    tx.result[3,] <- calculate_z_statistics(tx.output$diff.prob,
                                               tx.output$diff.se)
  ...
  if(sum(!is.null(ctrl.contrast1), !is.null(ctrl.contrast0))==2){
    ctrl.result[1,] <- calculate_z_statistics(ctrl.output$c1$mean.prob,
                                                 ctrl.output$c1$se)

    ctrl.result[2,] <- calculate_z_statistics(ctrl.output$c0$mean.prob,
                                                 ctrl.output$c0$se)

    ctrl.result[3,] <- calculate_z_statistics(ctrl.output$diff.prob,
                                                 ctrl.output$diff.se)
  ...
  if (!is.null(tx.contrast0) & !is.null(ctrl.contrast0)){
    ...
    base.result[1,] <- calculate_z_statistics(base.output$diff.prob,
                                                 base.output$diff.se)
  ...
  if (!is.null(tx.contrast1) & !is.null(ctrl.contrast1)){
    ...
    time1.result[1,] <- calculate_z_statistics(time1.output$diff.prob,
